...
(function($) {
	...
	Calendar.prototype = {
				...
				this.element.find('.month-container').each(function() {
					...
					if((_this.options.minDate == null || lastDate >= _this.options.minDate) && (_this.options.maxDate == null || firstDate <= _this.options.maxDate))
					{
						...
						for(var i in _this.options.dataSource) {
							if(!(_this.options.dataSource[i].checkIn > lastDate) || (_this.options.dataSource[i].checkOut < firstDate)) {
							...
							$(this).find('.day-content').each(function() {
								...
								if((_this.options.minDate == null || currentDate >= _this.options.minDate) && (_this.options.maxDate == null || currentDate <= _this.options.maxDate))
								{
									for(var i in monthData) {
										if(monthData[i].checkIn <= currentDate && monthData[i].checkOut >= currentDate){
									...
									if(dayData.length > 0)
									{
										var index = dayData.length - 1;
										if(start != dayData[index].id)
										{
											start = dayData[index].id;

											//check to see if multiple events on the same day
											if(index)
											{
												//overlaps
												// $(this).parent().css('background', '#ff9494');
											}
											else
											{
												//no overlaps
												$(this).parent().css('background', '#a9ec93')
												// $(this).parent().css('background', dayData[0].color);
											}

											//use below if you want all the same color (uncomment out above)
											// $(this).parent().css('background', dayData[0].color);
		...
		_renderDataSourceDay: function(elt, currentDate, events) {
			// if (currentDate.length == 0) {elt.parent().css('box-shadow', 'inset 0 -4px 0 0 red')};
			switch(this.options.style)
			{
				case 'border':
					...
					if(events.length == 1) {
						...
						elt.parent().css('background', '#a6f25a');
					}
				...
				case 'background':
					...
					if(events[events.length - 1].checkIn.getTime() == currentTime)
					{
						...
						if(events[events.length - 1].startHalfDay || this.options.alwaysHalfDay) {
							...
							for(var i = events.length - 2; i >= 0; i--) {
								if(events[i].checkIn.getTime() != currentTime || (!events[i].startHalfDay && !this.options.alwaysHalfDay)) {
					...
					else if(events[events.length - 1].checkOut.getTime() == currentTime)
					{
						...
						if(events[events.length - 1].endHalfDay || this.options.alwaysHalfDay) {
							...
							var otherColor = 'transparent';
							for(var i = events.length - 2; i >= 0; i--) {
								if(events[i].checkOut.getTime() != currentTime || (!events[i].endHalfDay &&  !this.options.alwaysHalfDay)) {
									otherColor = events[i].color;
									break;
								}
							}
			...
			if(this.options.enableRangeSelection) {
				...
				$(window).mouseup(function (e) {
					if (_this._mouseDown) {
						...
						_this._triggerEvent('selectRange', { checkIn: minDate, checkOut: maxDate });
					}
				});
			}
		...
		getEvents: function(date) {
			...
			if(this.options.dataSource && date) {
				for(var i in this.options.dataSource) {
					if(this.options.dataSource[i].checkIn <= date && this.options.dataSource[i].checkOut >= date) {
